...
int main (int argc, char *argv[])
{
 ...
 static const long long my_classifier_flags_solve = CRM114_LIBSVM;
  ...
  // static const long long my_classifier_flags = CRM114_FSCM;
   ...
   //static const long long my_classifier_flags = CRM114_ONENN;
  ...
  // static const long long my_classifier_flags = (CRM114_ENTROPY | CRM114_UNIQUE | CRM114_CROSSLINK);  // dynamic mesh + reuse
	//static const long long my_classifier_flags = CRM114_PCA;
  ...
  crm114_cb_setclassdefaults (p_cb);
   ...
   printf("Classifier flag is %d\n",(int)p_cb->classifier_flags);
...

{
 ...
 struct crm114_feature_row fr[FEATURE_SIZE];
 //struct crm114_feature_row fr1[FEATURE_SIZE];
 long nfr;		// number of struct crm114_feature_row
 long nfr1=0;		// modified number of struct crm114_feature_row
  ...
  nfr =FEATURE_SIZE;
	...
	whichclass--; //we know that in our data files classes are labeled as 1, 2 that's why we subtract
	//svm expects classes labeled as 0 and 1 
	
	/*	prob.y[docIndex] = (double)2.0*whichclass-1;
	    prob.x[docIndex] = &x_space[featureInd];
	    docIndex++;
	*/
	//printf("%d	",whichclass);
	...
	/* if ((fr = malloc(nfr * sizeof(struct crm114_feature_row))) == NULL)
				{
					printf("Cannot allocate, outside of while\n");
				}*/
		...
		while (!feof(fptr)) {
		    ...
		    /*  if (nfr > 0)	// realloc(0) -> free()
		      ...
		      err = crm114_learn_features(&p_db, whichclass, fr, &nfr);
		      		    
		     	free(fr);*/
		     	
		     	err = crm114_learn_features(&p_db, whichclass, fr, &nfr);
		     		   
		   //reinitiazating for the next line in the document.
		   
		   
		      /*nfr=FEATURE_SIZE;
		      if ((fr = malloc(nfr * sizeof(struct crm114_feature_row))) == NULL)
				{
					printf("Cannot allocate, in while\n");
				}*/
		      nfr1=0;
	...
	*/
			...
			fr[nfr1].feature=(unsigned)temp;
			nfr1++;
			
			/*x_space[featureInd].index =temp;
			x_space[featureInd].value = 1.0;
		...
		//	printf("Temp %d	",temp);
			if (temp == -2)
			{
		     ...
		     nfr = nfr1;
		      ...
		      err = crm114_classify_features(p_db, fr, &nfr, &result);
		      crm114_show_result("Results", &result); 		    
		     
		     if(whichclass == result.bestmatch_index)
		     	correct++;
		     total++;
		      ...
		      nfr1=0;
				...
				whichclass--; //we know that in our data files classes are labeled as 1, 2 that's why we subtract
						//svm expects classes labeled as 0 and 1 
				//printf("which class %d	",whichclass);
				result.true_class = whichclass;
				//printf("true class is : %d	\n",result.true_class);
				continue;
			}
			fr[nfr1].feature=(unsigned)temp;
			nfr1++; 	
