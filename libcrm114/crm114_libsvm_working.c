...
//This function saves the features in the training set into hashes document by document.
...
CRM114_ERR crm114_learn_features_libsvm_solve (CRM114_DATABLOCK **db)
    ...
    //For more details see the pdf titled as "A practical Guide for SVM classification" http://www.csie.ntu.edu.tw/~cjlin/papers/guide/guide.pdf
  ...
  //allocating enough space for the feature row vector that stores all the features in the training corpus.  
...
featureIndex =0;
    ...
    {		
      ...
      //   get a pointer to the known features hash table
      		ht_scan = (HYPERSPACE_FEATUREBUCKET *) (&(*db)->data[0] +  (*db)->cb.block[classInd].start_offset);
      		upt=0;
      		unsigned valueStore;
    		while (upt < htlen)
				{
	...
	//remove the duplicates from the (feature densification table) and sort the features according to the has values
    ...
    // crm114__resize_a_block (db, whichblock, (size_t)par*sizeof(int));
    ...
    for (j=0;j<par;j++)
    {
    	valueStore = allFeatures[j].feature;
