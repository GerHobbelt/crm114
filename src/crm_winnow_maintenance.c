			    ...
			    unsigned long hindex)
{
  ...
  if (packstart > packend) packlen = (hs - packstart ) + (packend );
  ...
  crm_pack_winnow_css (h, seen_features, hs, packstart, packlen);
  ...
  while (zcountdown > 0)
      ...
      for (k = start; k <= end;  k++)
	{
	  if (h[k].key != 0 )       // key == 0 means "special- don't zero!"
		...
		{
		  //           zeroize it totally - it should not be there.
		  h[k].hash = 0;
		  h[k].key  = 0;
		  h[k].value = 0;
		};
			  ...
			  long hs, long packstart, long packlen)
{
  ...
  if (packstart+packlen <= hs)   //  no wraparound in this case
    ...
    {
      crm_pack_winnow_seg (h, seen_features, hs, packstart, (hs - packstart));
      crm_pack_winnow_seg (h, seen_features, hs, 1, (packlen - (hs - packstart)));
    };
}
			  ...
			  long hs, long packstart, long packlen)
{
  ...
  unsigned long thash, tkey, tseen;
  float tvalue;
		       ...
		       long major, long minor, long spectrum_start)
{
  FILE *f;
  long i;
  WINNOW_FEATUREBUCKET_STRUCT feature = {0, 0, 0.0};
