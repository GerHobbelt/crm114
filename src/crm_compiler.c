...
break;
	...
	if (internal_trace)
	{
		...
		for (stmtnum = 0; stmtnum < numstmts; stmtnum++)
		{
			...
			fprintf(stderr, "\n      args:\n");
			if (apb->a1len) fprintf(stderr, "        angles 1:      <%.*s>\n", apb->a1len, apb->a1start);
			if (apb->b1len) fprintf(stderr, "        boxes 1:       [%.*s]\n", apb->b1len, apb->b1start);
			if (apb->p1len) fprintf(stderr, "        parentheses 1: (%.*s)\n", apb->p1len, apb->p1start);
			if (apb->p2len) fprintf(stderr, "        parentheses 2: (%.*s)\n", apb->p2len, apb->p2start);
			if (apb->p3len) fprintf(stderr, "        parentheses 3: (%.*s)\n", apb->p3len, apb->p3start);
			if (apb->s1len) fprintf(stderr, "        slashes 1:     /%.*s/\n", apb->s1len, apb->s1start);
			if (apb->s2len) fprintf(stderr, "        slashes 2:     /%.*s/\n", apb->s2len, apb->s2start);
		}
	}
