...
static void add_token_count(CLUSTEROR_STATE_STRUCT *s, index_t t, COOCCURRENCE_SCORE_TYPE c)
{
    ...
    if (s->tokens[t].cor_index == NULL_INDEX)
    {
        ...
        else                    //we're still filling cor token slots
        {
            ...
            if (s->header->least_frequent_cor_token == NULL_INDEX || count <=
                s->tokens[s->header->least_frequent_cor_token].count)
            {
        ...
        regex_t *regee)
{
    ...
    for (*n = 0; *n < max; (*n)++)
    {
        ...
        *t++ = get_token_from_hash(s, strnhash(&text[match.rm_so], l));
        ...
        if (0 && internal_trace
            && *(t - 1) != simple_hash_lookup(s, strnhash(&text[match.rm_so], l)))
        {
            fatalerror_ex(SRC_LOC(),
...
static void make_new_clusteror_state(CLUSTEROR_STATE_STRUCT *s)
{
    ...
    s->header->graph_offset = 4 + s->header->cooccurences_offset + max_cor_tokens *
                              (max_cor_tokens + 3) / 2 * sizeof(COOCCURRENCE_SCORE_TYPE);
        ...
        regex_t *regee)
{
    ...
    for (*n = 0; *n < max; (*n)++)
    {
        ...
        *t++ = simple_hash_lookup(s, strnhash(&text[match.rm_so], l));
        ...
        if (0 && internal_trace)
        {
            fprintf(stderr, "no_learning_get_token_from_hash returned %ld\n", *(t - 1));
        }
