 ...
 *
 * Currently it takes 20 minutes and 3 seconds to learn the first
...
static void add_occ(CLUSTEROR_STATE_STRUCT *s, index_t i,
                    COOCCURRENCE_SCORE_TYPE a);

//  Likewise for co-occurences
static COOCCURRENCE_SCORE_TYPE get_cooc(CLUSTEROR_STATE_STRUCT *s, index_t i,
                                        index_t j);
static void add_cooc(CLUSTEROR_STATE_STRUCT *s, index_t i, index_t j,
                     COOCCURRENCE_SCORE_TYPE a);
...
static void delete_from_hash(CLUSTEROR_STATE_STRUCT *s, index_t token,
                             long key);
...
(CLUSTEROR_STATE_STRUCT *s,
 index_t *t, int *n, int max, char *text, long len, regex_t *regee);
...
//  sincrementally, so instead we marked the bit set token_nn_changed
...
//  look!
static void score_document_mix(CLUSTEROR_STATE_STRUCT *s, index_t *doc,
                               long len, COOCCURRENCE_SCORE_TYPE sense);
static void score_document_sharp(CLUSTEROR_STATE_STRUCT *s, index_t *doc,
                                 long len, COOCCURRENCE_SCORE_TYPE sense);
static void score_document_gauss(CLUSTEROR_STATE_STRUCT *s, index_t *doc,
                                 long len, COOCCURRENCE_SCORE_TYPE sense);
static void score_document_flat(CLUSTEROR_STATE_STRUCT *s, index_t *doc,
                                long len, COOCCURRENCE_SCORE_TYPE sense);
...
static void add_token_count(CLUSTEROR_STATE_STRUCT *s, index_t t,
                            COOCCURRENCE_SCORE_TYPE c);
...
#ifdef NOTUSED
static COOCCURRENCE_SCORE_TYPE score_cooccurance_quick_and_dirty(
    CLUSTEROR_STATE_STRUCT *s,
    index_t                i,
    index_t                j)
{
...
#ifdef NOTUSED
static COOCCURRENCE_SCORE_TYPE score_cooccurance_slow_and_clean(
    CLUSTEROR_STATE_STRUCT *s,
    index_t                i,
    index_t                j)
{
...
static COOCCURRENCE_SCORE_TYPE score_cooccurance_slow_and_clean2(
    CLUSTEROR_STATE_STRUCT *s,
    index_t                i,
    index_t                j)
{
    double N = get_N(s),
           ...
           PijHyp = Pi * Pj,
           PijHat = get_cooc(s, i, j) / N,
           L = PijHat / PijHyp, correlation = crm_log(L);

    return (COOCCURRENCE_SCORE_TYPE)correlation;
}

static COOCCURRENCE_SCORE_TYPE score_cooccurance(CLUSTEROR_STATE_STRUCT *s,
                                                 index_t i, index_t j)
{
...
static void add_occ(CLUSTEROR_STATE_STRUCT *s,
                    index_t i, COOCCURRENCE_SCORE_TYPE a)
{
...
static void add_cooc(CLUSTEROR_STATE_STRUCT *s,
                     index_t i, index_t j, COOCCURRENCE_SCORE_TYPE a)
{
    ...
    if (ci == cj)
    {
                fatalerror("We're pooped!","Two tokens have same correlation index!\n");
    }
    if (ci < cj)
    {
        index_t t = i;
    ...
    if (a > 0)
    {
        ...
        if (s->cor_tokens[ci].nearest_neihbor == NULL_INDEX
           || score_cooccurance(s,i,j) > score_cooccurance(s, 
           i, 
           s->cor_tokens[ s->cor_tokens[ci].nearest_neihbor ].token)    )
        {
...
static index_t grab_edge(CLUSTEROR_STATE_STRUCT *s)
{
    ...
    {
        fatalerror("We're pooped!",
            "We've run out of graph edges. This should never happen! PANIC! The pigeon hole principle no longer holds!\n");
    }
    return a;
}
...
static void set_edge(CLUSTEROR_STATE_STRUCT *s, index_t i, index_t j, int e)
{
    ...
    {
        fatalerror("We're pooped!",
                        "someone tried to make a loop in the graph!!!\n");
    }
                             ...
                             long key)
{
    ...
    for ( ; ;)
    {
        ...
        {
            if (0 && internal_trace && s->hash_table[i].key != key)
            {
                ...
                for (k = ((unsigned long)key) % first_hash_level_size;
                    ...
                    fprintf(
                        ...
                        k,
                        s->hash_table[k].key,
                        s->hash_table[k].token,
                        s->hash_table[k].next_in_hash_chain);
            }
...
static void wipe_cooccurences(CLUSTEROR_STATE_STRUCT *s, index_t t)    //t is the cor_index
{
    ...
    {
        fatalerror("We're pooped!",
                        "Tried to qipe a stupid token number!\n");
    }
...
static void audit_frequency_list(CLUSTEROR_STATE_STRUCT *s)
{
         ...
         a = s->tokens[a].more_common, i++)
    {
        ...
        for (b = s->header->least_frequent_token, j = 0; 
        b != a && j < i;
             b = s->tokens[b].more_common, j++) 
             ;
        if (j < i)
        {
            fprintf(
                            ...
                            COOCCURRENCE_SCORE_TYPE c)
{
        ...
        if (t == s->header->least_frequent_cor_token)
            do
                s->header->least_frequent_cor_token =
                    s->tokens[s->header->least_frequent_cor_token].more_common;
            while (s->tokens[s->header->least_frequent_cor_token].cor_index ==
                   NULL_INDEX);
        if (s->tokens[t].less_common != NULL_INDEX)
    ...
    if (s->tokens[t].cor_index == NULL_INDEX)
    {
        ...
        if (s->header->next_free_cor_token == MAX_COR_TOKENS)
        {
            ...
            {
                s->tokens[t].cor_index =
                    s->tokens[s->header->least_frequent_cor_token].cor_index;
                s->tokens[s->header->least_frequent_cor_token].cor_index =
                    NULL_INDEX;

                while (s->tokens[s->header->least_frequent_cor_token].
                       cor_index == NULL_INDEX)
                    s->header->least_frequent_cor_token =
                        s->tokens[s->header->least_frequent_cor_token].
                        more_common;
        ...
        else                    //we're still filling cor token slots
        {
          ...
          if(s->header->least_frequent_cor_token == NULL_INDEX 
          || count <= s->tokens[ s->header->least_frequent_cor_token ].count )
...
static void hash_insert(CLUSTEROR_STATE_STRUCT *s, long hash_key,
                        index_t token)
{
    long first_hash_level_size = s->header->max_tokens;
    index_t i =
        (index_t)(((unsigned long)hash_key) % first_hash_level_size);
    index_t j =
        NULL_INDEX;
    ...
    for ( ; i != NULL_INDEX; j = i, i = s->hash_table[i].next_in_hash_chain)  //go to end of hash chain
        ...
        fatalerror_ex(SRC_LOC(),
                    "Attempting to insert (%ld : %lu) ontop of (%ld : %lu)\n",
...
static void diagnose_hash_table(CLUSTEROR_STATE_STRUCT *s)
{
    ...
    for (i = 0; i < s->header->max_tokens; i++)
        if (s->hash_table[i].token != NULL_INDEX)
        {
            ...
            for (j = i;
                 j != NULL_INDEX;
                 j = s->hash_table[j].next_in_hash_chain)
            {
                c++;
                l++;
            }
    ...
    for (i = s->header->first_unused_hash_slot; 
    i != NULL_INDEX;
         i = s->hash_table[i].next_in_hash_chain)
...
static void learning_tokenize(CLUSTEROR_STATE_STRUCT *s,
                              ...
                              char *text, long len, regex_t *regee)
{
    int l, i;
    regmatch_t match;
    ...
    for (*n = 0; *n < max; (*n)++)
    {
        ...
        *t++ = get_token_from_hash(s, strnhash(&text[match.rm_so], l));
        ...
        if (0 && internal_trace
            && *(t - 1) != simple_hash_lookup(s,
                                              strnhash(&text[match.rm_so], l)))
        {
        fatalerror_ex(SRC_LOC(),
                "Failed to relookup token! hash: 0x%08lX token: %ld gotback: %ld\n",
                (unsigned long)strnhash(&text[match.rm_so], l),
                *(t - 1),
...
static void make_new_clusteror_state(CLUSTEROR_STATE_STRUCT *s)
{
    long i, max_tokens = s->header->max_tokens, max_cor_tokens =
        s->header->max_cor_tokens;
    ...
    s->header->hash_slots_offset +=
        BYTE_ALIGN - s->header->hash_slots_offset % BYTE_ALIGN;

    s->header->tokens_offset =
        s->header->hash_slots_offset +
        2 * max_tokens * sizeof(HASH_NODE_STRUCT);
    s->header->tokens_offset +=
        BYTE_ALIGN - s->header->tokens_offset % BYTE_ALIGN;

    s->header->cor_tokens_offset =
        s->header->tokens_offset + max_tokens * sizeof(TOKEN_STRUCT);
    s->header->cor_tokens_offset +=
        BYTE_ALIGN - s->header->cor_tokens_offset % BYTE_ALIGN;

    s->header->cooccurences_offset =
        s->header->cor_tokens_offset +
        max_cor_tokens * sizeof(COR_TOKEN_STRUCT);
    s->header->cooccurences_offset +=
        BYTE_ALIGN - s->header->cooccurences_offset % BYTE_ALIGN;

    s->header->graph_offset =
        4 + s->header->cooccurences_offset + max_cor_tokens * (max_cor_tokens +
                                                               3) / 2 *
        sizeof(COOCCURRENCE_SCORE_TYPE);
    s->header->graph_offset +=
        BYTE_ALIGN - s->header->graph_offset % BYTE_ALIGN;

    s->header->clusters_offset =
        s->header->graph_offset + (3 * max_cor_tokens +
                                   2) * sizeof(EDGE_STRUCT);
    s->header->clusters_offset +=
        BYTE_ALIGN - s->header->clusters_offset % BYTE_ALIGN;

    s->hash_table =
        (HASH_NODE_STRUCT *)(((char *)(s->header)) +
                             s->header->hash_slots_offset);
    s->tokens =
        (TOKEN_STRUCT *)(((char *)(s->header)) + s->header->tokens_offset);
    s->cor_tokens =
        (COR_TOKEN_STRUCT *)(((char *)(s->header)) +
                             s->header->cor_tokens_offset);
    s->cooccurences =
        (COOCCURRENCE_SCORE_TYPE *)(((char *)(s->header)) +
                                    s->header->cooccurences_offset);
    s->graph =
        (EDGE_STRUCT *)(((char *)(s->header)) + s->header->graph_offset);
    s->clusters =
        (CLUSTER_STRUCT *)(((char *)(s->header)) + s->header->clusters_offset);
...
static void map_file_for_learn(CLUSTEROR_STATE_STRUCT *s, char *filename)
{
    ...
    if (stat(filename, &statbuf))        //if the file didn't exist already
    {
        ...
        file_size +=
            n_cor_tokens * (n_cor_tokens +
                            3) / 2 * sizeof(COOCCURRENCE_SCORE_TYPE);
        file_size += BYTE_ALIGN - file_size % BYTE_ALIGN;
        ...
        if (internal_trace)
        {
            if (!test_memory_consistancy
                ((unsigned char *)(s->header), file_size))
            {
        fatalerror("We're pooped!",
                        "Memory map found to be inconsistent!\n");
            }
            else
                        {
        fatalerror("We're pooped!",
                        "Memory map found to be consistent\n");
                        }
        }
        ...
        {
        fatalerror_ex(SRC_LOC(),
            "Couldn't map new file %s! errno = %d(%s)\n", filename,
                    errno,
                                        errno_descr(errno));
        }
        CRM_ASSERT(s->header != NULL);
    ...
    else
    {
        ...
        {
        fatalerror_ex(SRC_LOC(),
             "Couldn't map existing file %s! errno = %d(%s)\n",
...
static void find_NNs_for_unlearning(CLUSTEROR_STATE_STRUCT *s)
{
    ...
    for (i = 0; i < s->header->n_cor_tokens; i++)
    {
        if (get_bit(s->token_nn_changed, i))
        {
            ...
            {
                p = score_cooccurance(s, s->cor_tokens[i].token,
                                      s->cor_tokens[j].token);
                if (i != j && p > m)
                {
                    m = p;
                    n = j;
    ...
    for (i = 0; i < s->header->n_tokens; i++)
        if (get_bit(s->closed_list, i))
        {
            ...
            if (s->tokens[i].more_recent < NULL_INDEX)
                s->tokens[s->tokens[i].more_recent].less_recent =
                    s->tokens[i].less_recent;
            if (s->tokens[i].less_recent < NULL_INDEX)
                s->tokens[s->tokens[i].less_recent].more_recent =
                    s->tokens[i].more_recent;
                                 ...
                                 long len, COOCCURRENCE_SCORE_TYPE sense)
{
    ...
    for (i = 0; i < len; i++)
        for (j = i + 1; j < i + WINDOW_SIZE && j < len; j++)
            add_cooc(s, doc[i], doc[j],
                     sense / (COOCCURRENCE_SCORE_TYPE)(j - i));
}
                                 ...
                                 long len, COOCCURRENCE_SCORE_TYPE sense)
{
    ...
    for (i = 0; i < len; i++)
        for (j = i + 1; j < i + WINDOW_SIZE && j < len; j++)
            add_cooc(s, doc[i], doc[j],
                     sense * normalized_gauss((COOCCURRENCE_SCORE_TYPE)(j - i),
                                              gauss_sigma));
}
    ...
    *stack_ptr++ = i;
    while (stack_ptr > stack)
    {
        ...
        if (!get_bit(s->closed_list, a))
        {
            ...
            if (a == j)
                break;
            for (b = s->cor_tokens[a].edges; b != NULL_INDEX;
                 b = s->graph[b].next)
...
static void change_cluster(CLUSTEROR_STATE_STRUCT *s, index_t i, index_t c)
{
    ...
    if (s->cor_tokens[i].cluster != NULL_INDEX)
        s->clusters[s->cor_tokens[i].cluster].occurrences -=
            get_occ(s, s->cor_tokens[i].token);
        ...
        if (internal_trace)
        {
            ...
            for (i = s->header->first_unused_cluster_slot, j = 0;
              ...
              i = s->clusters[i].next_free, j++)
                ...
                {
        fatalerror_ex(SRC_LOC(),
                     "giving back cluster %ld twice, arg!!!\n",
                            c);
                }
            if (i == NULL_INDEX)
            {
        fatalerror("We're pooped!",
                    "There's a cluster in use AND in the free cluster list, arg!!!\n");
            }
            if (j == MAX_CLUSTERS)
            {
        fatalerror("We're pooped!",
                        "There's a loop in the free cluster list, arg!!!\n");
            }
        }
...
static void join_clusters(CLUSTEROR_STATE_STRUCT *s, index_t i, index_t j)
{
    ...
    {
        fatalerror_ex(SRC_LOC(),
        "Made to join identical tokens!\n nn(%ld) = %ld\n", i,
                s->cor_tokens[i].nearest_neihbor);
    }
    ...
    if (s->cor_tokens[i].cluster == s->cor_tokens[j].cluster
        && s->cor_tokens[i].cluster != NULL_INDEX)
    {
        if (internal_trace)
            fputc('I', stderr);
...
static void speculative_split_clusters(CLUSTEROR_STATE_STRUCT *s, index_t i,
                                       index_t j)
{
    index_t c;
    index_t a, b;
...
static int verify_graph(CLUSTEROR_STATE_STRUCT *s)
{
    long i, j, c = 0, d = 0;
...
static int test_bit_vectors(void)
{
    long b, i;
...
//  is undefined.
//  Anything besides digits and key names can be used to
//  delimit values which are placed in an array of doubles called values,
//  default values should already be in it

static void parse_monster(char *text, long len, const char **keys,
                          double *values)
{
    regex_t regee;
    ...
    while (*keys)
    {
        ...
        regee_text_len =
            strmov(regee_text + regee_text_len,
        ...
        else
        {
            ...
            {
                for (b = bufee, i = match[1].rm_so;
                     i < match[1].rm_eo;
                     b++, i++)
                {
                    CRM_ASSERT(i < MAX_PATTERN);
                    *b = text[i];
                }
...
//  it's nearest neighbor by the score
int crm_expr_clump_nn(CSL_CELL *csl, ARGPARSE_BLOCK *apb)
{
    ...
    const char *keys[] =
    {
        "max_tokens", "max_cor_tokens", "flat_weight", "sharp_weight",
        "gauss_weight", "gauss_sigma", (char *)0
    };
    /* values[] will be edited by parse_monster(): */
    double values[] = 
        {
        MAX_TOKENS, MAX_COR_TOKENS, 0.0, 1.0, 0.0, 1.0 
        };
    ...
    parameters_text_len =
        crm_nexpandvar(parameters_text, parameters_text_len, MAX_PATTERN);
    ...
    {
        fatalerror_ex(SRC_LOC(),
        "Problem compiling this regex %s\n", regex_text);
    }

    crm_get_pgm_arg(box_text, MAX_PATTERN, apb->b1start, apb->b1len);
    if (0 >
        crm_restrictvar(box_text, apb->b1len, NULL, &text, &text_start,
                        &text_len, errstr))
    {
        fatalerror_ex(SRC_LOC(),
        "Error grabbing text! %s\n", errstr);
    }
    ...
    else
    {
        ...
        learning_tokenize(&s, tokenized_text, &tokenized_text_len,
                          text_len / 2 + 1, text + text_start, text_len,
                          &regee);
...
"Unable to open cluster file %s\n", filename);
    ...
    s.hash_table =
        (HASH_NODE_STRUCT *)((char *)(s.header) + s.header->hash_slots_offset);
    s.tokens = (TOKEN_STRUCT *)((char *)(s.header) + s.header->tokens_offset);
    s.cor_tokens =
        (COR_TOKEN_STRUCT *)((char *)(s.header) + s.header->cor_tokens_offset);
    if (internal_trace)
    {
        fprintf(
            stderr,
    ...
    for (i = 0; i < tokenized_text_len; i++)
    {
        ...
        if (t == NULL_INDEX)
            ...
            out_pos +=
                sprintf(outbuf + out_pos, "%ld ",
                        s.cor_tokens[s.tokens[t].cor_index].cluster);
    }
    CRM_ASSERT(outbuf[out_pos] == 0);
...
static void poke_around(CLUSTEROR_STATE_STRUCT *s)
{
    ...
    fprintf(stdout, "s->header->n_tokens = %ld\n", s->header->n_tokens);
    fprintf(stdout, "s->header->n_clusters = %ld\n", s->header->n_clusters);

    for (i = 0; i < s->header->n_cor_tokens; i++)
        for (j = i + 1; j < s->header->n_cor_tokens; j++)
        {
            a = score_cooccurance(s, s->cor_tokens[i].token,
                                  s->cor_tokens[j].token);
            if (a < min)
                min = a;
            if (a > max)
    ...
    fprintf(stdout, "Cooccurences:\n\tmin: %0.3f\tmax:%0.3f\n", min, max);
    for (k = 0, b = min; k < N_BINS; k++, b += bb)
        fprintf(stdout, "\t(%0.4f, %0.4f):\t%ld\n", b, b + bb, bins[k]);
    for (i = 0; i < N_BINS; i++)
        bins[i] = 0;
    bb = (max - min) / N_BINS;
    for (i = 0; i < s->header->n_cor_tokens; i++)
    {
        a = score_cooccurance(s, s->cor_tokens[i].token,
                              s->cor_tokens[s->cor_tokens[i].nearest_neihbor].
                              token);
        for (k = 0, b = bb + min; k < N_BINS; k++, b += bb)
            if (a <= b)
            {
    ...
    fprintf(stdout, "Nearest neighbor cooccurences:\n");
    for (k = 0, b = min; k < N_BINS; k++, b += bb)
        fprintf(stdout, "\t(%0.4f, %0.4f):\t%ld\n", b, b + bb, bins[k]);
    ...
    fprintf(stdout, "cor_token Occurences:\n\tmin: %0.3f\tmax:%0.3f\n", min, max);
    for (k = 0, b = min; k < N_BINS; k++, b += bb)
        fprintf(stdout, "\t(%0.3f, %0.3f):\t%ld\n", b, b + bb, bins[k]);
}
