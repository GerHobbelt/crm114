...
if [ ! -f "${scriptfile}" ]; then
  ...
  if [ "x`echo "${scriptfile}" | sed -e "s/^-{.\+} */-{/" `" = "x-{" ]; then
    report_msg "going to use '${scriptfile}' as literal test script"
    literalscript=1
  else
...
if [ ! -z "${expected}" ]; then
  if [ "x${expected}" = "x-" ]; then
    if [ ${literalscript} = 0 ]; then
      ...
      report_msg "going to use expected outfile file '${expected}'..."
    else
...
report_msg "going to run CRM114 '${scriptfile}' with commandline: ${args}"

retcode=65536
if [ -z "${inputdata}" ]; then
  report_msg "input is nil/empty"
  report_msg ": crm114 ${scriptfile} ${args}"
  ${CRM_BIN} "${scriptfile}" ${args} 2> "${tmperr}" > "${tmpout}"
  retcode=$?
else
  ...
  if [ -f "${inputdata}" ]; then
    # run test:
    report_msg ": crm114 ${scriptfile} ${args}"
    ${CRM_BIN} "${scriptfile}" ${args} < "${inputdata}" 2> "${tmperr}" > "${tmpout}"
    retcode=$?
  else
    report_msg ": echo '${inputdata}' | crm114 ${scriptfile} ${args}"
    echo "${inputdata}" | ${CRM_BIN} "${scriptfile}" ${args} 2> "${tmperr}" > "${tmpout}"
  fi
fi
...
retcode=65536
if [ ! -z "${diffscript}" ]; then
  "${diffscript}" "${expected}" "${tmpchk}" "${diffout}"
  retcode=$?
else
