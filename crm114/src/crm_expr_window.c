...
#define EOFRETRY 1
    ...
    mdw = NULL;
    if (vht[vmidx]->valtxt == cdw->filetext)
        mdw = cdw;
    ...
    while (!done)
    {
        ...
        case FROM_VAR:
            {
                ...
                //     and push the contents of the variable into inputtxt
                strncpy(inputtxt,
                        &vht[srcidx]->valtxt[vht[srcidx]->vstart],
                        inputtxtlen);
                newinputbuf[inputtxtlen] = 0;
                //
                //    and there we have it - intputtxt has all we will
        ...
        case FROM_STDIN:
            {
                    ...
                    default:
                        {
                            ...
                            //          icount = read (0, &(inputtxt[inputtxtlen]), 1);
                            //
                            if (user_trace)
                                fprintf(stderr, "   single character BYCHAR read\n");
                            icount = (int)fread(&inputtxt[inputtxtlen], 1, 1, stdin);
                        }
                        break;
                    }
                }
                ...
                {
                    inputtxtlen += icount;
                    inputtxt[inputtxtlen] = 0; // put on the terminator
                }
        ...
        //      now have an inputtxt with something worth examining
        //     in it, of length inputtxtlen (i.e. using chars [0...inputtxtlen-1])
        //
    ...
    //     Once we get to here, we have the new input in inputtxt, and
...
invoke_bailout:
    ...
    //
    //    Last little bit of cleanup: write remainder back to variable (instead of buffering for stdin).
    //
    if (inputsrc == FROM_VAR)
    {
